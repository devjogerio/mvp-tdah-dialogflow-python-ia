...
# O AssistantService encapsula a escolha do LLM (Bedrock, Gemini, OpenAI) via variável de ambiente
...
def lambda_handler(event, context):
    ...
    try:
        ...
        is_dialogflow = "queryResult" in (json.loads(event.get("body", "{}")) if isinstance(
            event.get("body"), str) else event.get("body", {}))

        if not user_message:
            error_msg = "Mensagem vazia ou formato inválido."
            logger.warning(error_msg)
            response_body = DialogflowAdapter.format_response(
                error_msg, is_dialogflow)
            return {
                "statusCode": 400,
        ...
        logger.info(
            f"Processando mensagem: '{user_message}' [Sessão: {session_id}]")
        ...
        if not is_safe:
            logger.warning(
                f"Conteúdo de risco detectado na sessão {session_id}.")
            response_body = DialogflowAdapter.format_response(
                emergency_msg, is_dialogflow)
        ...
        context_data = assistant_service.retrieve_context(user_message)

        # 4. Geração de Resposta (LLM)
        response_text = assistant_service.invoke_model(
            user_message, context_data)

        # 5. Formatação de Saída
        response_body = DialogflowAdapter.format_response(
            response_text, is_dialogflow)
        if not is_dialogflow:
        ...
        try:
            is_dialogflow_error = "queryResult" in (json.loads(event.get(
                "body", "{}")) if isinstance(event.get("body"), str) else event.get("body", {}))
        except:
        ...
        response_body = DialogflowAdapter.format_response(
            error_text, is_dialogflow_error)

        return {
            # Dialogflow precisa de 200 com texto de erro
            "statusCode": 200 if is_dialogflow_error else 500,
            "body": json.dumps(response_body)
        }
