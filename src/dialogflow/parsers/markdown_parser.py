...
class CaseStudyParser:
    ...
    def __init__(self):
        ...
        self.entities_map = {
            "Especialista": ["Psiquiatra", "Neurologista", "Clínico Geral", "Psicólogo"],
            "Sintoma": ["desânimo", "falta de motivação", "baixa autoestima", "esquecimentos", "desorganização"],
            "Tratamento": ["Antidepressivos", "Estimulantes", "TCC", "Terapia Cognitivo-Comportamental", "Psicoeducação"]
        }
    ...
    def _extract_intents(self, content: str) -> List[Dict]:
        ...
        for section in sections:
            ...
            for key, phrases in self.topic_triggers.items():
                if key.lower() in title_line.lower() or clean_title.lower() in key.lower():
                    triggers = phrases
                    break

            if not triggers:
                triggers = [f"Falar sobre {clean_title}",
